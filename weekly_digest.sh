#!/bin/bash
# launchd가 매주 호출하는 진입점 — subscribe_server.py(Railway)의 관리자 엔드포인트를
# 트리거만 한다. publish.sh와 똑같은 이유로 PATH를 직접 고정한다(launchd는 로그인 셸
...
LOGFILE="$PROJECT_DIR/logs/weekly_digest_$(date +%Y%m%d_%H%M%S).log"

{
  echo "=== $(date '+%Y-%m-%d %H:%M:%S') 주간 다이제스트 발송 트리거 ==="
  ENDPOINT=$(grep '^SUBSCRIBE_ENDPOINT=' .env | cut -d= -f2 | sed 's#/subscribe$##')
  SECRET=$(grep '^UNSUB_SECRET=' .env | cut -d= -f2)
  curl -sf -X POST "$ENDPOINT/admin/send-weekly-digest?token=$SECRET"
  echo
