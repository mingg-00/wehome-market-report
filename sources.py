...
#
# "실증특례"·"규제샌드박스"도 같은 문제다 — 이건 공유숙박 전용 제도가 아니라
# 전 산업 공통 규제 샌드박스 프로그램 명칭이라, 핀테크·모빌리티·통신 등 다른
# 업종 기사에도 똑같이 등장한다. 2026-07-30 실측: "갤럭시폰이 보이스피싱범
# 목소리 잡는다…성문 탐지 실증특례" 기사(통신·보안 분야)가 이 단어 하나로
# 공유숙박 태그가 붙었다. 위홈 자체 블로그 글("공유숙박 실증특례와 임시허가,
# 무엇이 다를까요?")처럼 숙박 관련 단어가 근처에 있을 때만 진짜 매치로 인정한다.
